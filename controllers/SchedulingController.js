...
exports.scheduling = function(req, res, next) {
  ...
  var sql = 'select T0."ContractID" as "ContractID", T1."ItemName" as "ItemName", T1."InternalSN" as "SerialNum", T2."status" as "Status" from octr T0 join ctr1 T1 on T0."ContractID"=T1."ContractID" join oins T2 on T1."InsID" = T2."insID"where "InternalSN" in (' + req.body.sn + ') and T1."TermDate" is null and now() < T1."EndDate" and T2."status" = ' + "'" + 'A' + "'" + '';
