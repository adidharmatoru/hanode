...
exports.sqssdetail = function(req, res, next) {
  ...
  var sql = 'select   T1."DocNum", T1."CardCode",T1."Series",  SUBSTRING(T1."DocDueDate",1,10) as "DocDueDate",  T1."Address",  T3."Tel1",  T3."Tel2",  T3."E_MailL",  T5."ItemCode",   T5."Dscription", T5."Price",T1."DocType" ,  T5."Quantity" from   oqut T1   left join nnm1 T2 on T1."Series" = T2."Series"   left join qut1 T5 on T1."DocEntry" = T5."DocEntry"   left join ocpr T3 on T1."CntctCode" = T3."CntctCode" where T1."DocStatus" = ' + "'" + 'O' + "'" + ' and left(T2."SeriesName", 4) = ' + "'" + 'SQSS' + "'" + ' and T1."DocEntry" = ' + "' " + req.query.code + " '" + '';
