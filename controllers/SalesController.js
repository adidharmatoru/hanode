...
exports.reportpenjualan = function(req, res, next){
  ...
  var sql = 'select   T1."CardName" as "customer",   T2."Dscription" as "Item",   T1."DocNum" AS "No_SQ",   SUBSTRING(T1."DocDate", 1, 10) as "Date_SQ",   T2."Price" as "Harga_SQ",   T2."Quantity" as "Qty_SQ",   T1."DocTotal" as "Total_SQ",   R2."DocNum" as "No_SO",   SUBSTRING(R2."DocDate", 1, 10) as "Date_SO",   R1."Price" as "Harga_SO",   R2."DocTotal" as "Total_SO",   R1."Quantity" as "Qty_SO",   P2."DocNum" as "No_DP",   SUBSTRING(P2."DocDate", 1, 10) as "Date_DP",   P1."Price" as "Harga_DP",   P1."Quantity" as "Qty_DP",   P2."DocTotal" as "Total_DP",   D2."DocNum" as "No_DO",   SUBSTRING(D2."DocDate", 1, 10) as "Date_DO",   V2."DocNum" as "No_Inv",   SUBSTRING(V2."DocDate", 1, 10) as "Date_Inv",   V1."Price" as "Harga_Inv",   V1."Quantity" as "Qty_Inv",   V2."DocTotal" as "Total_Inv",   ct1."DocNum" as "No_IP",   SUBSTRING(ct1."DocDate", 1, 10) as "Date_IP",   ct1."DocTotal" as "Total_IP",   T1."OwnerCode" as "OwnerCode",   H1."lastName" || ' + "'" + '  ' + "'" + ' || H1."firstName" as "NameFullOwner"  from oqut T1 	left join qut1 T2 on T1."DocEntry" = T2."DocEntry"	left join rdr1 R1 on T2."DocEntry" = R1."BaseEntry" 	left join ordr R2 on R1."DocEntry" = R2."DocEntry"	left join dpi1 P1 on R1."DocEntry" = P1."BaseEntry" and P1."BaseType" = 17	left join odpi P2 on P1."DocEntry" = P2."DocEntry"	left join dln1 D1 on R1."DocEntry" = D1."BaseEntry" and D1."BaseType" = 17	left join odln D2 on D1."DocEntry" = D2."DocEntry"	left join inv1 V1 on R1."DocEntry" = V1."BaseEntry" and V1."BaseType" = 17 or D1."DocEntry" = V1."BaseEntry" and V1."BaseType" = 15	left join oinv V2 on V1."DocEntry" = V2."DocEntry"	left join rct2 ct2 on V2."DocEntry" = ct2."DocEntry" or P1."DocEntry" = ct2."DocEntry" and P2."ReceiptNum" = ct2."DocNum" 	left join orct ct1 on ct2."DocNum" = ct1."DocEntry" left join ohem H1 on T1."OwnerCode" = H1."empID" where 	SUBSTRING(T1."DocDate", 1, 10) BETWEEN ' + "'" + req.query.startdate + "'" + '  AND ' + "'" + req.query.enddate + "'" + ' and T1."CardName" = ' + "'" + req.query.customer + "'" + '  and T1."OwnerCode" = ' + "'" + req.query.owner + "'" + ' order by T1."DocNum" desc';
