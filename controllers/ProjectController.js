...
exports.itemmonitoringPO = function (req, res, next){
  ...
  var sql = 'SELECT    distinct T0."Project" as "PrjCode",	left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson", T1."CardName" as "Customer", S1."SlpName" as "SalesName", T1."NumAtCard" as "PO", prj."U_Status" as "Status", dln1."Quantity" as "QuantityDO" FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join oslp S1 on T1."SlpCode" = S1."SlpCode"  left join dln1 dln1 on T0."Project" = dln1."Project"   left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left join inv1 inv1 on prj."PrjCode" = inv1."Project"     left join oinv oinv on inv1."DocEntry" = oinv."DocEntry"   left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" where  T0."Price" != 0 and T0."Project" != '+ "' '" +' and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and prj."U_Status" in ('+ req.body.status +') AND NOT dln."DocStatus" = ' + "'" + 'C' + "'" + ' or T0."Price" != 0 and T0."Project" != '+ "' '" +' and prj."U_Status" in ('+ req.body.status +') and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' AND NOT dln."DocStatus" = ' + "'" + 'C' + "'" + ' order by T0."Project"';
