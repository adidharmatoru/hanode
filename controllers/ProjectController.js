...
exports.primarySoldItems1 = function(req, res, next) {
  ...
  var sql = 'SELECT T0."Project" as "PrjCode",T1."DocNum" as "SODocNum", T1."U_VIT_WGSE" as "WarrantyService", left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson", T1."CardName" as "Customer", T1."NumAtCard" as "PO", T0."ItemCode" as "ItemCode", T0."Dscription" as "ItemName", T0."Quantity", T0."Price", T0."LineTotal",  T1."U_VIT_TOPY", nnm1."BeginStr" || ' + "'-'" + ' || dln."DocNum" as "Delivery No", dln."U_VIT_NPAN" as "Delivery Address", left(dln."DocDate", 10) as "DeliveryDate", dln."U_VIT_DIOL" as "Receiver" , T0."SerialNum" as "SerialNumber", T1."CardCode" as "CardCode"  FROM RDR1 T0 JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry" and T0."LineNum" = dln1."BaseLine" and T0."Project" = dln1."Project" left join odln dln on dln1."DocEntry" = dln."DocEntry" left join oinv inv on T1."DocEntry" = inv."DocEntry" left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" left join nnm1 on dln."Series" = nnm1."Series" left join oprj prj on T0."Project" = prj."PrjCode" where T0."Project" in (' + req.query.prj + ') and T1."U_VIT_WGSE" is not null order by T0."Project"';
...
exports.itemmonitoring = function (req, res, next){
  ...
  var sql = 'SELECT    distinct T0."Project" as "PrjCode", T0."PrjName" as "PrjName", 	left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson", T1."CardName" as "Customer", S1."SlpName" as "SalesName", T1."NumAtCard" as "PO", prj."U_Status" as "Status", prj."U_StartDate" as "U_StartDate", oinv."DocStatus" as "Invoice Status", T1."U_VIT_TOPY", dln."DocNum" as "Delivery", left(dln."DocDueDate", 10) as "DeliveryDate", dln."Address" as "AlamatPengiriman", dln1."ItemCode" as "ItemCodeDO", dln1."Dscription" as "ItemNameDO", dln1."Quantity" as "QuantityDO",dln1."SerialNum" as "SerialNumber", dln."U_VIT_DIOL" as "Receiver" FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join oslp S1 on T1."SlpCode" = S1."SlpCode"  left join dln1 dln1 on T0."Project" = dln1."Project"   left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left join inv1 inv1 on prj."PrjCode" = inv1."Project"     left join oinv oinv on inv1."DocEntry" = oinv."DocEntry"   left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" where  T0."Price" != 0 and T0."Project" != '+ "' '" +' and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and prj."U_Status" in ('+ req.body.status +') and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' or T0."Price" != 0 and T0."Project" != '+ "' '" +' and prj."U_Status" in ('+ req.body.status +') and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' order by T0."Project"';
...
exports.itemmonitoringPO = function (req, res, next){
  ...
  var sql = 'SELECT   distinct T0."Project" as "PrjCode", 	left(T1."DocDate", 10) as "SO Date", 	T2."SlpName" as "SalesPerson", 	T1."CardName" as "Customer", 	S1."SlpName" as "SalesName", 	T1."NumAtCard" as "PO", 	prj."U_Status" as "Status"FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry"   left join oslp S1 on T1."SlpCode" = S1."SlpCode"   left join dln1 dln1 on T0."Project" = dln1."Project"   left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" where  T0."Price" != 0 and T0."Project" != '+ "' '" +' and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and prj."U_Status" in ('+ req.body.status +') or T0."Price" != 0 and T0."Project" != '+ "' '" +' and prj."U_Status" in ('+ req.body.status +') and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' order by T0."Project"';
...
exports.monitoring = function (req, res, next){
  ...
  var sql = 'SELECT    distinct T0."Project" as "PrjCode", T0."PrjName" as "PrjName", 	left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson", T1."CardName" as "Customer", S1."SlpName" as "SalesName", T1."NumAtCard" as "PO", prj."U_Status" as "Status", prj."U_StartDate" as "U_StartDate", oinv."DocStatus" as "Invoice Status", T1."U_VIT_TOPY", dln."DocNum" as "Delivery", left(dln."DocDueDate", 10) as "DeliveryDate", dln."Address" as "AlamatPengiriman", dln1."ItemCode" as "ItemCodeDO", dln1."Dscription" as "ItemNameDO", dln1."Quantity" as "QuantityDO", ins."internalSN" as "SerialNumber", ins."insID" as "insID", dln1."SerialNum" as "SerialNumber", dln."U_VIT_DIOL" as "Receiver" FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join oslp S1 on T1."SlpCode" = S1."SlpCode"  left join dln1 dln1 on T0."Project" = dln1."Project" left join oins ins on dln1."DocEntry" = ins."delivery"  left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left join inv1 inv1 on prj."PrjCode" = inv1."Project"     left join oinv oinv on inv1."DocEntry" = oinv."DocEntry"   left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" where  T0."Price" != 0 and T0."Project" != '+ "' '" +' and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and prj."U_Status" in ('+ req.body.status +') and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' AND dln1."BaseLine"=' + "'" + '0' + "'" + ' or T0."Price" != 0 and T0."Project" != '+ "' '" +' and prj."U_Status" in ('+ req.body.status +') and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' AND dln1."BaseLine"=' + "'" + '0' + "'" + ' order by T0."Project"';
