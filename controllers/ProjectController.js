...
exports.primarySoldItems1 = function(req, res, next) {
  ...
  var sql = 'SELECT T0."Project" as "PrjCode",T1."DocNum" as "SODocNum", dln."U_VIT_WGSE" as "WarrantyService", T1."U_VIT_WGSE" as "WarrantyServiceSO", left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson", T1."CardName" as "Customer", T1."NumAtCard" as "PO", T0."ItemCode" as "ItemCode", T0."Dscription" as "ItemName", T0."Quantity", T0."Price", T0."LineTotal",  T1."U_VIT_TOPY", nnm1."BeginStr" || ' + "'-'" + ' || dln."DocNum" as "Delivery No", dln."U_VIT_NPAN" as "Delivery Address",dln."Address" as "Delivery Address2", left(dln."DocDate", 10) as "DeliveryDate", dln."U_VIT_DIOL" as "Receiver" , T0."SerialNum" as "SerialNumber", T1."CardCode" as "CardCode"  FROM RDR1 T0 JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry" and T0."LineNum" = dln1."BaseLine" and T0."Project" = dln1."Project" left join odln dln on dln1."DocEntry" = dln."DocEntry" left join oinv inv on T1."DocEntry" = inv."DocEntry" left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" left join nnm1 on dln."Series" = nnm1."Series" left join oprj prj on T0."Project" = prj."PrjCode" where T0."Project" in (' + req.query.prj + ') and dln."U_VIT_WGSE" is not null and dln."CANCELED" = '+ "'N'" +' or T0."Project" in (' + req.query.prj + ') and T1."U_VIT_WGSE" is not null and dln."CANCELED" = '+ "'N'" +' order by T0."Project"';
...
exports.scall = function(req, res, next) {
  ...
  var sql = 'SELECT   T0."contractID" as "ContractNo",   T0."subject" as "Subject",   T0."BPShipAddr" as "address",   T0."itemName" as "ItemName",   T0."itemCode" as "ItemCode",   T0."internalSN" as "internalSN",   T0."U_VIT_NOSO" as "No SO",   T0."Telephone" as "tlpn",   T0."callID" as "SC_no",   SUBSTRING(T0."createDate", 1, 10) as "SC_date",   SUBSTRING(T0."cntrctDate", 1, 10) as "ContractDate",   T0."DocNum" as "DocNum",   T5."Name" as "ContactName",   SUBSTRING(T0."closeDate", 1, 10) as "SC_enddate",   T4."SeriesName" as "SeriesName",   T0."U_KS_REMARK" as "Remark",   T0."Location" as "Location",   T0."custmrName" as "Name"FROM   OSCL T0   left join scl1 T2 ON T0."callID" = T2."srvcCallID"   left join nnm1 T4 on T0."Series" = T4."Series"   left join ocpr T5 on T5."CntctCode" = T0."contctCode" order by   T0."DocNum" desc';
