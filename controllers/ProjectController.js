...
exports.itemmonitoring = function (req, res, next){
  ...
  var sql = 'SELECT   distinct T0."Project" as "PrjCode",   T1."DocNum" as "SO DocNum",   oinv."DocStatus" as "Invoice Status",   left(T1."DocDate", 10) as "SO Date",   T2."SlpName" as "SalesPerson",   T1."CardName" as "Customer",   T1."NumAtCard" as "PO", T0."ItemCode" as "ItemCode",   case T1."DocType" when ' + "' " + 'I' + " '" + ' then T0."Dscription" else case when length(T0."Dscription") > coalesce(    length(T0.u_vit_ft),     0  ) then T0."Dscription" else TO_VARCHAR(T0.U_VIT_FT) end end as "ItemName",   case T1."DocType" when ' + "' " + 'I' + " '" + ' then T0."Quantity" else T0."U_VIT_QT" end as "Quantity",   T0."Price",   T0."LineTotal",   T1."U_VIT_TOPY",   dln."DocNum" as "Delivery",   dln."Address" as "AlamatPengiriman",   left(dln."DocDueDate", 10) as "DeliveryDate",   dln."U_VIT_DIOL" as "Receiver" FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry"   left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry"   and T0."LineNum" = dln1."BaseLine"   and T0."Project" = dln1."Project"   left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left join inv1 inv1 on prj."PrjCode" = inv1."Project"   left join oinv oinv on inv1."DocEntry" = oinv."DocEntry"   left join OSLP T2 on T1."SlpCode" = T2."SlpCode" where T0."Price" != 0 order by   T0."Project"';
