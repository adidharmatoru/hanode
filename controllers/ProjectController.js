...
exports.primarySoldItems2 = function(req, res, next) {
  ...
  var sql = 'SELECT distinct T0."Project" as "PrjCode", T1."DocNum" as "SO DocNum", oinv."DocStatus" as "Invoice Status", oinv."CANCELED" as "cancel",  left(T1."DocDate", 10) as "SO Date",T2."SlpName" as "SalesPerson",T1."CardName" as "Customer",T1."NumAtCard" as "PO",T0."ItemCode" as "ItemCode",case T1."DocType" when ' + "'" + 'I' + "'" + ' then T0."Dscription" else case when length(T0."Dscription") > coalesce(length(T0.u_vit_ft),	0) then T0."Dscription" else TO_VARCHAR(T0.U_VIT_FT) end end as "ItemName", case T1."DocType" when ' + "'" + 'I' + "'" + ' then T0."Quantity" else T0."U_VIT_QT" end as "Quantity",T0."Price",T0."LineTotal", T1."U_VIT_TOPY", dln."DocNum" as "Delivery", left(dln."DocDueDate", 10) as "DeliveryDate",dln."Address" as "Delivery Address2", dln."U_VIT_DIOL" as "Receiver", oinv."DocTotal" as "inv_total", T1."DocTotal" as "so_total", oinv."DocNum" as "DocNum_INV", ct1."DocNum" as "DocNum_incoming", ct2."SumApplied" as "jumlah_incoming", P0."DocNum" as "NumDP", P0."DocTotal" as "dptotal" FROM RDR1 T0 JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry" and T0."LineNum" = dln1."BaseLine" and T0."Project" = dln1."Project" left join odln dln on dln1."DocEntry" = dln."DocEntry" left join oprj prj on T0."Project" = prj."PrjCode" left join inv1 inv1 on prj."PrjCode" = inv1."Project" left join oinv oinv on inv1."DocEntry" = oinv."DocEntry" left join dpi1 P1 on T0."DocEntry" = P1."BaseEntry" left join odpi P0 on P1."DocEntry" = P0."DocEntry" left join rin1 R1 on P1."DocEntry" = R1."BaseEntry" left join rct2 ct2 on oinv."DocEntry" = ct2."DocEntry" or P1."DocEntry" = ct2."DocEntry" and P0."ReceiptNum" = ct2."DocNum" left join orct ct1 on ct2."DocNum" = ct1."DocEntry" left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" where T0."Project" in (' + req.body.prj + ') and T0."Price" != 0 and R1."DocEntry" is null order by T0."Project"';
...
var sql =  'select distinct  A."DistNumber",   A."ItemCode",   A."itemName",   SUBSTRING(Y."DocDate", 1, 10) as "DoDate", SUBSTRING(Y."DocDueDate", 1, 10) as "DocDueDate", Y."DocNum" as "NoDO",   SUBSTRING(F."DocDate", 1, 10) as "SoDate",   F."DocNum" as "NoSO",   Y."U_VIT_WGSA" as "Warranty-SparePart",   Y."U_VIT_WGBA" as "Warranty-Battery", Y."U_VIT_WGSE" as "Warranty-Service" from   OSRN A   INNER JOIN ITL1 B ON A."ItemCode" = B."ItemCode"   AND A."SysNumber" = B."SysNumber"   INNER JOIN OITL C ON B."LogEntry" = C."LogEntry"   INNER JOIN odln Y on C."DocType" = Y."ObjType"   AND C."DocEntry" = Y."DocEntry"   INNER JOIN dln1 X on X."DocEntry" = Y."DocEntry"   AND C."DocLine" = X."LineNum"   INNER JOIN rdr1 Z on X."BaseEntry" = Z."DocEntry"   INNER JOIN ordr F on Z."DocEntry" = F."DocEntry" where   X."Project" = ' + "'" + req.query.prj + "'" + ' and A."ItemCode" NOT LIKE ' + "'" + '%CLOSE RACK%' + "'" + ' and A."ItemCode" NOT LIKE ' + "'" + '%PANEL%' + "'" + '';
