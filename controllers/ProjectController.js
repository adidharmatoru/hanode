...
exports.primarySoldItems1 = function(req, res, next) {
  ...
  var sql = 'SELECT T0."Project" as "PrjCode",T1."DocNum" as "SODocNum", dln."U_VIT_WGSE" as "WarrantyService", T1."U_VIT_WGSE" as "WarrantyServiceSO", left(T1."DocDate", 10) as "SO Date", T2."SlpName" as "SalesPerson",T1."CardCode" as "CardCode", T1."CardName" as "Customer", T1."NumAtCard" as "PO", T0."ItemCode" as "ItemCode", T0."Dscription" as "ItemName", T0."Quantity", T0."Price", T0."LineTotal",  T1."U_VIT_TOPY", nnm1."BeginStr" || ' + "'-'" + ' || dln."DocNum" as "Delivery No", dln."U_VIT_NPAN" as "Delivery Address",dln."Address" as "Delivery Address2", left(dln."DocDate", 10) as "DeliveryDate", dln."U_VIT_DIOL" as "Receiver" , T0."SerialNum" as "SerialNumber", T1."CardCode" as "CardCode",IFNULL(cnt."Tel1",cnt."Tel2") as "ContactTel",cnt."CntctCode" as "contactCode"  FROM RDR1 T0 JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry" and T0."LineNum" = dln1."BaseLine" and T0."Project" = dln1."Project" left join odln dln on dln1."DocEntry" = dln."DocEntry" left join oinv inv on T1."DocEntry" = inv."DocEntry" left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" left join nnm1 on dln."Series" = nnm1."Series" left join oprj prj on T0."Project" = prj."PrjCode" left join ocpr cnt on dln."CntctCode" = cnt."CntctCode" where T0."Project" in (' + req.query.prj + ') and dln."U_VIT_WGSE" is not null and dln."CANCELED" = '+ "'N'" +' or T0."Project" in (' + req.query.prj + ') and T1."U_VIT_WGSE" is not null and dln."CANCELED" = '+ "'N'" +' order by T0."Project"';
