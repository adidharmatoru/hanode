...
exports.itemmonitoring = function (req, res, next){
  ...
  var sql = 'SELECT   distinct T0."Project" as "PrjCode",   oinv."DocStatus" as "Invoice Status", T1."DocNum" as "SO DocNum",   left(T1."DocDate", 10) as "SO Date",   T2."SlpName" as "SalesPerson",   T1."CardName" as "Customer",   T1."NumAtCard" as "PO",   prj."U_Status" as "Status",   prj."U_StartDate" as "U_StartDate",   T1."U_VIT_TOPY",   dln."DocNum" as "Delivery",   left(dln."DocDueDate", 10) as "DeliveryDate",   dln."Address" as "AlamatPengiriman",   dln1."ItemCode" as "ItemCodeDO",   dln1."Dscription" as "ItemNameDO",   dln1."Quantity" as "QuantityDO",   dln."U_VIT_DIOL" as "Receiver" FROM   RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry"   left join dln1 dln1 on T0."DocEntry" = dln1."BaseEntry"   and T0."LineNum" = dln1."BaseLine"   and T0."Project" = dln1."Project"   left join odln dln on dln1."DocEntry" = dln."DocEntry"   left join oprj prj on T0."Project" = prj."PrjCode"   left join inv1 inv1 on prj."PrjCode" = inv1."Project"   left join oinv oinv on inv1."DocEntry" = oinv."DocEntry"   left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" left join inv1 inv1 on prj."PrjCode" = inv1."Project" left join oinv oinv on inv1."DocEntry" = oinv."DocEntry" where  T0."Price" != 0 and T0."Project" != '+ "' '" +' and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and prj."U_Status" in ('+ req.body.status +') and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' AND NOT dln."DocStatus" = ' + "'" + 'C' + "'" + ' or T0."Price" != 0 and T0."Project" != '+ "' '" +' and prj."U_Status" in ('+ req.body.status +') and SUBSTRING(prj."U_StartDate", 1, 10) BETWEEN ' + "'" + req.body.year + "'" + ' AND  ' + "'" + req.body.yer + "'" + ' and NOT dln1."ItemCode"= ' + "'" + 'IM' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'IC' + "'" + ' AND NOT dln1."ItemCode" =' + "'" + 'SC' + "'" + ' AND NOT dln."DocStatus" = ' + "'" + 'C' + "'" + ' order by T0."Project"';
