...
exports.serialNum = function(req, res)
{
  ...
  var sql ='  select   ns."customer",   ns."custmrName",   ns."instLction",   ns."itemName",   ns."itemCode",   ns."internalSN",   ns."delivery",  ns."deliveryNo",  d1."U_VIT_WGSA" as "WarrantySparePart",   d1."U_VIT_WGBA" as "WarrantyBattery",  SUBSTRING(d1."DocDate", 1, 10) as "PostingDate",   pr."CntctCode",   pr."Tel1",   pr."Name", t1."ContractID", SUBSTRING(t1."StartDate", 1, 10), SUBSTRING(t1."EndDate", 1, 10) from   oins ns   left join ocpr pr on ns."customer" = pr."CardCode" left join ctr1 t1 on ns."internalSN" = t1."InternalSN" left join odln d1 on ns."delivery" = d1."DocEntry"where   "internalSN" LIKE ' + "'%" + req.query.code + "%'" + ' and t1."TermDate" is null and t1."EndDate" > CURRENT_DATE';
