...
exports.customerall = function(req, res, next) {
  ...
  var sql = 'select "CardCode", "CardName","Phone1" from OCRD where "validFor"=' + "'Y'" + '  order by "CardName" DESC';
...
exports.SconItem = function(req, res, next) {
  ...
  res.setHeader('Access-Control-Allow-Credentials', true); // If needed
    ...
    var sql = 'SELECT   distinct T0."ContractID" as "contractID",   T0."ItemCode" as "Code_Item",   T0."Line" as "LineNum",  T0."ItemName" as "Item_Name",   T0."U_FH_MNT" AS "Visit",  T0."InternalSN" as "SerinalNum",   T3."custmrName" as "customerName",   T3."instLction" as "Location",   SUBSTRING(T0."StartDate", 1, 10) as "StartDate",T4."insID" as "insID",   SUBSTRING(T0."EndDate", 1, 10) as "EndDate",  T4."Name" as "Name",   T4."Tel1" as "phone",   T4."E_MailL" as "E_mail" FROM   CTR1 T0   JOIN OCTR T1 ON T1."ContractID" = T0."ContractID"   left join oins T3 ON T0."InsID" = T3."insID"   left join ocpr T4 ON T3."contactCod" = T4."CntctCode"  where T0."ContractID" in (' + req.body.cid + ') and T3."status"='+ "'A'" +'  order by T0."ContractID"';
