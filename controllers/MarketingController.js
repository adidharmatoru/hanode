...
exports.SconItem = function(req, res, next) {
  ...
  // var sql = 'SELECT distinct T0."Project" as "PrjCode", T1."DocNum" as "SO DocNum", left(T0."DocDate", 10) as "SO Date",T2."SlpName" as "SalesPerson",T1."CardName" as "Customer",T1."NumAtCard" as "PO",T0."ItemCode" as "ItemCode",T0."Dscription" as "ItemName",dln1."Quantity",dln1."Price",T0."LineTotal", T1."U_VIT_TOPY", dln."DocNum" as "Delivery", left(dln."DocDate", 10) as "DeliveryDate", dln."U_VIT_DIOL" as "Receiver"FROM RDR1 T0 JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry" left join dln1 dln1 on T0."Dscription" = dln1."Dscription" and T0."Project" = dln1."Project" left join odln dln on dln1."DocEntry" = dln."DocEntry" left join oinv inv on T1."DocEntry" = inv."DocEntry" left JOIN OSLP T2 ON T2."SlpCode" = T1."SlpCode" left join oprj prj on T0."Project" = prj."PrjCode" where T0."Project" is not null and T0."Project" != ' + "'" + "'" +' and (T0."Dscription" NOT LIKE  ' + "'%EATON" + "%'" + ' or T0."Dscription" NOT LIKE  ' + "'%LKPP" + "%'" + 'or T0."Dscription" NOT LIKE  ' + "'%PENGIRIMAN" + "%'" + 'or T0."Dscription" NOT LIKE  ' + "'%PANEL" + "%'" + ') and left(prj."U_StartDate", 4) = year(CURRENT_DATE) and dln."CANCELED" != ' + "'Y" + "'" +'order by T0."Project"';
    ...
    var sql = 'SELECT distinct T0."ContractID" as "contractID", T0."ItemCode" as "Code_Item", T0."ItemName" as "Item_Name", T0."InternalSN" as "SerinalNum", T3."custmrName" as "customerName", T3."instLction" as "Location" FROM CTR1 T0 JOIN OCTR T1 ON T1."ContractID" = T0."ContractID" left join oins T3 ON T0."InsID" = T3."insID" where T0."ContractID" in (' + req.body.cid + ') and T3."status"='+ "'A'" +'  order by T0."ContractID"';
...
exports.ScallDetail = function(req, res, next) {
  ...
  var sql = 'select T0."callID" as "SC_no", T0."custmrName" as "Name",T0."BPShipAddr" as "Address",T0."internalSN" as "SerialNum",T0."itemCode" as "ItemCode",T0."itemName" as "ItemName",T0."U_VIT_BMRK" as "MerkBatterai",T0."U_VIT_BMOD" as "ModelBatterai",SUBSTRING(T0."U_VIT_BDAT",1,10) as "DateBatterai",T0."U_VIT_BPCS" as "BatteraiPCS",T0."U_VIT_TINR" as "Tegangan_in_R",T0."U_VIT_TINS" as "Tegangan_in_S",T0."U_VIT_TINT" as "Tegangan_in_T",T0."U_VIT_TOUR" as "Tegangan_out_R",T0."U_VIT_TOUS" as "Tegangan_out_S",T0."U_VIT_TOUT" as "Tegangan_out_T",SUBSTRING(T1."StartDate",1,10) as "StartDate",SUBSTRING(T1."EndDate",1,10) as "EndDate",T4."SeriesName" as "SeriesName",T0."DocNum" as "DocNum",SUBSTRING(T0."createDate",1,10) as "DateCreate",T2."Name" as "ContactName",T0."Telephone" as "tlpn",T0."subject" as "Subject",T0."U_KS_REMARK" as "Remark" from oscl T0 join octr T1 on T0."contractID"=T1."ContractID" LEFT JOIN ocpr T2 on T2."CntctCode"=T0."contctCode" join nnm1 T4 on T0."Series" = T4."Series" where T0."callID" = ' + "'" + req.query.code + "'" + '';
