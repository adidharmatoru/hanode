...
exports.so = function(req, res, next) {
  ...
  var sql = 'select "CardCode","DocEntry","DocNum","CardName","Address","NumAtCard","DocTotal","CntctCode" from ordr where "Series"= ' + "'1674'" + ' and "CardCode" in (' + "'" + req.query.code + "'" + ') order by "DocNum" DESC';
...
exports.teknisi = function(req, res, next) {
  ...
  var sql = 'select "empID", "lastName" as "LastName","firstName" as "FirstName", "jobTitle" as "JobTitle" from ohem where "jobTitle" in (' + "'TEKNISI'" + ',' + "'TEKNISI SOLO'" + ',' + "'TEKNISI MAINTENANCE'" + ',' + "'TEKNISI INSTALASI'" + ',' + "'TEKNISI REPAIR'" + ') order by "jobTitle" asc';
...
exports.Scall = function(req, res, next) {
  ...
  var sql = 'SELECT distinct T0."contractID" as "ContractNo",SUBSTRING(T0."DateCreate",1,10) as "DateCreate",T3."SltCode" as "SltCode","status", as "CloseDate", T0."callID" as "SC_no", SUBSTRING(T0."createDate",1,10) as "SC_date", SUBSTRING(T0."cntrctDate",1,10) as "SC_enddate" FROM OSCL T0 join scl1 T2 ON T0."callID"=T2."srvcCallID" join oslt T3 ON T2."solutionID"=T3."SltCode" where T0."contractID" in (' + req.body.cid + ') order by T0."contractID"';
