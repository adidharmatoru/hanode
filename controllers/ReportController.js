...
exports.pdcaSales = function(req, res) {
  ...
  var sql = 'SELECT T10."DocNum" "Quot. No.", T0."DocEntry", T0."DocNum" As "SO No.", left(T10."DocDate", 10) as "DocDate", left(T10."DocDueDate", 10) as "DocDueDate", T5."DocEntry" as "DownPayment No.", T0."CardCode", T0."CardName", T0."NumAtCard", T1."ItemCode", T1."Dscription", T1."FreeTxt",T1."Quantity", T1."Price" as "SO Price", T4."PymntGroup" "Payment terms", T3."DocNum" As "DLN No.", T2."Quantity" As "Total Rcvd", T1."OpenQty", left(T1."ShipDate", 10) as "ShipDate", T8."DocNum" "Inv. No." FROM ORDR  T0 INNER JOIN RDR1 T1 ON T0."DocEntry" = T1."DocEntry" LEFT Join DLN1 t2 On T1."DocEntry"=T2."BaseEntry" and T1."LineNum"=T2."BaseLine"  LEFT Join ODLN T3 On T2."DocEntry"=T3."DocEntry"  Left Join OCTG T4 On T0."GroupNum" = T4."GroupNum"  LEFT JOIN DPI1 T5 On T5."BaseEntry"=T0."DocEntry"  LEFT JOIN ODPI T6 On T5."DocEntry"=T6."DocEntry" LEFT Join INV1 t7 On T2."DocEntry"=T7."BaseEntry" and T2."LineNum"=T7."BaseLine"  LEFT Join OINV T8 On T7."DocEntry"=T8."DocEntry" LEFT Join QUT1 t9 On T1."DocEntry"=T9."TrgetEntry" and T9."LineNum"=T1."BaseLine"  LEFT Join OQUT T10 On T9."DocEntry"=T10."DocEntry" Where T8."DocStatus"=' + "'" + 'C' + "'" + ' and left(T10."DocDate", 4) = year(CURRENT_DATE) Group By T0."DocNum", T10."DocDate", T10."DocDueDate", T0."CardCode", T0."CardName", T0."NumAtCard", T1."ItemCode", T1."Dscription", T1."Quantity", T1."OpenQty", T3."DocNum", T2."Quantity", T1."Price", T4."PymntGroup", T1."FreeTxt", T0."DocEntry", T1."ShipDate", T5."DocEntry", T8."DocNum", T10."DocNum" Order By T0."DocEntry"';
