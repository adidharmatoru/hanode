...
exports.pdcaSales = function(req, res) {
  ...
  var sql = 'SELECT T10."DocNum" "Quot. No.", ordr."DocEntry", ordr."DocNum" As "SO No.", left(T10."DocDate", 10) as "DocDate", left(T10."DocDueDate", 10) as "DocDueDate", dpi1."DocEntry" as "DownPayment No.", ordr."CardCode", ordr."CardName", ordr."NumAtCard", case when T10."DocType" =' + "'" + 'I' + "'" + ' then qut1."ItemCode" else qut1."Dscription" end as "ItemCode", qut1."Dscription" as "Dscription", case when T10."DocType" =' + "'" + 'I' + "'" + ' then qut1."Quantity" else qut1.U_VIT_QT end as "Quantity", case when t10."DocType" =' + "'" + 'I' + "'" + ' then qut1."Price" else qut1.U_VIT_UP end as "Item Price", octg."PymntGroup" "Payment terms", odln."DocNum" As "DLN No.", case when odln."DocType" =' + "'" + 'I' + "'" + ' then dln1."Quantity" else dln1."U_VIT_QT" end As "Total Rcvd", rdr1."OpenQty", left(rdr1."ShipDate", 10) as "ShipDate", rdr1."LineNum", oinv."DocNum" "Inv. No.", oinv."DocTotal" "Inv. Total", T10."DocTotal" "SQ Total", ohem."lastName", ohem."firstName", ohem."middleName" FROM ORDR  ordr INNER JOIN RDR1 rdr1 ON ordr."DocEntry" = rdr1."DocEntry" LEFT Join DLN1 dln1 On rdr1."DocEntry"=dln1."BaseEntry" and rdr1."LineNum"=dln1."BaseLine"  LEFT Join ODLN odln On dln1."DocEntry"=odln."DocEntry" Left Join OCTG octg On ordr."GroupNum" = octg."GroupNum" LEFT JOIN DPI1 dpi1 On dpi1."BaseEntry"=ordr."DocEntry" LEFT JOIN ODPI odpi On dpi1."DocEntry"=odpi."DocEntry" LEFT Join INV1 inv1 On dln1."DocEntry"=inv1."BaseEntry" and dln1."LineNum"=inv1."BaseLine"  LEFT Join OINV oinv On inv1."DocEntry"=oinv."DocEntry" LEFT Join QUT1 qut1 On rdr1."DocEntry"=qut1."TrgetEntry" and qut1."LineNum"=rdr1."BaseLine"  LEFT Join OQUT T10 On qut1."DocEntry"=T10."DocEntry" join ohem ohem on T10."OwnerCode" =  ohem."empID" Where oinv."DocStatus" is not null and ordr."NumAtCard" = odln."NumAtCard" and ordr."NumAtCard" = oinv."NumAtCard" and left(T10."DocDate", 4) in  (' + req.query.year + ') Group By ordr."DocNum", T10."DocDate", T10."DocDueDate", ordr."CardCode", ordr."CardName", ordr."NumAtCard", qut1."ItemCode", qut1."Dscription", qut1."Quantity", rdr1."OpenQty", odln."DocNum", dln1."Quantity", qut1."Price", octg."PymntGroup", rdr1."FreeTxt", ordr."DocEntry", odln."DocDate", dpi1."DocEntry", oinv."DocNum", oinv."DocTotal", T10."DocTotal", T10."DocNum", ohem."lastName", ohem."firstName", ohem."middleName", t10."DocType", qut1.U_VIT_QT, qut1.U_VIT_UP, DLN1.U_VIT_QT, ODLN."DocType", rdr1."ShipDate", rdr1."LineNum" Order By ordr."DocEntry"';
