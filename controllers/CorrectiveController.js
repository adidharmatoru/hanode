...
exports.corrective = function(req, res, next) {
  ...
  var sql = 'select   T0."DocNum" as "Nomor SC",   T0."callID",   T0."custmrName",   T0."customer" as "CardCode",   T0."subject" as "subject",   N1."SeriesName" as "SeriesName",   N1."Series" as "Series",   T1."Name",   T2."DocAbs",   T5."Name" as "contactperson",   T0."itemName" as "ItemName",   T0."itemCode" as "ItemCode",   T0."internalSN" as "SerialNum",   T0."BPShipAddr" as "Address",   T0."BPPhone1" as "Phone",    T0."BPE_Mail" as "E_mail",   T0."BPShipAddr" as "Address",  SUBSTRING(T0."createDate", 1, 10) as "createDate",   SUBSTRING(T0."closeDate", 1, 10) as "closeDate",   AT."AbsEntry" as "AbsEntry",   AT."FileExt" as "FileExt",   AT."FileName" as "FileName",   T0."U_KS_REMARK" as "Remark",   AT."trgtPath" as "targetPath",   T0."status" as "Status" from   oscl T0   join oscs T1 on T0."status" = T1."statusID"   left join scl4 T2 on T0."callID" = T2."SrcvCallID"   left join ocpr T5 on T0."contctCode" = T5."CntctCode"   left join ATC1 AT on AT."AbsEntry" = T0."AtcEntry"   left join nnm1 N1 on T0."Series" = N1."Series"  where T0."Series" in (' + req.query.series + ') order by   T0."createDate" DESC';
