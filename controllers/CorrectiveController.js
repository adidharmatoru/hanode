...
exports.salesQuotation = function(req, res, next) {
  ...
  var sql = 'select distinct T3."DocNum" as "DocNum", T2."TrgetEntry" as "TargetEntry", T3."DocEntry" as "DocEntry",SUBSTRING(T3."CreateDate",1,10) as "CreateDate" from qut1 T2 left join oqut T3 on T2."DocEntry"=T3."DocEntry" where T3."Series"in ('+"'1720'"+','+ "'2149'"+') and T3."DocEntry" in (' + req.body.sq + ')';
...
exports.delivery= function(req, res, next) {
  ...
  res.setHeader('Access-Control-Allow-Credentials', true); // If needed
    ...
    var sql = 'select distinct T3."DocNum" as "DocNum", T3."DocEntry" as "DocEntry",T4."SrcvCallID" as "callID",SUBSTRING(T3."CreateDate",1,10) as "CreateDateDO" from dln1 T2 left join odln T3 on T2."DocEntry"=T3."DocEntry" left join scl4 T4 on T2."BaseEntry"=T4."DocAbs" where T3."Series" in ('+"'1963'"+','+"'1615'"+','+"'1346'"+') and T2."DocEntry" in (' + req.body.dlv + ')';
