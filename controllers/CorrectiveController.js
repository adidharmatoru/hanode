...
exports.corrective = function(req, res, next) {
  ...
  var sql = 'select   T0."DocNum" as "Nomor SC",   T0."callID",   T0."custmrName", T0."customer" as "CardCode",  T0."subject" as "subject",   N1."SeriesName" as "SeriesName", N1."Series" as "Series",  T1."Name",  T2."DocAbs",   T5."Name" as "contactperson",   T4."DocNum" as "NomorSO",   T0."itemName" as "ItemName",   T0."itemCode" as "ItemCode",   T0."internalSN" as "SerialNum",   T0."BPShipAddr" as "Address",   T0."BPPhone1" as "Phone",   T4."U_VIT_TOPY" as "TOP",   T4."DocTotal" as "Total",   T0."BPE_Mail" as "E_mail",  T0."BPShipAddr" as "Address",  SUBSTRING(T7."DocDate", 1, 10) as "DocDateDO",   T7."DocNum" as "DocNumDO",   SUBSTRING(T4."DocDate", 1, 10) as "DocDateSO",   T4."DocNum" as "DocNumSO",   SUBSTRING(T0."createDate", 1, 10) as "createDate",   SUBSTRING(T0."closeDate", 1, 10) as "closeDate",   AT."AbsEntry" as "AbsEntry",   AT."FileExt" as "FileExt",   AT."FileName" as "FileName", T0."U_KS_REMARK" as "Remark" , AT."trgtPath" as "targetPath", T0."status" as "Status" from   oscl T0   join oscs T1 on T0."status" = T1."statusID"   left join scl4 T2 on T0."callID" = T2."SrcvCallID"   left join rdr1 T3 on T2."DocAbs" = T3."DocEntry" or T2."DocAbs" = T3."BaseEntry"  left join ordr T4 on T3."DocEntry" = T4."DocEntry"   left join ocpr T5 on T0."contctCode" = T5."CntctCode"   left join dln1 T6 on T2."DocAbs" = T6."BaseEntry"   left join odln T7 on T6."DocEntry" = T7."DocEntry"   left join ATC1 AT on AT."AbsEntry" = T0."AtcEntry"   left join nnm1 N1 on T0."Series" = N1."Series" where    T0."Series" in (' + req.query.series + ') order by   T0."createDate" DESC limit 5000';
...
exports.Inv = function(req, res, next) {
  ...
  var sql = 'select distinct T0."DocNum" as "SO_no", T3."DocNum" as "no_inv", T0."U_VIT_TOPY" as "TOP", T3."CardName" as "CardNameInvoice", SUBSTRING(T3."DocDate", 1, 10) as "inv_date", T3."DocStatus" as "status", T3."DocTotal" as "Total",T5."DocNum" as "no_dp",T5."DocTotal" as "Total_dp" from ordr T0 join rdr1 T1 on T0."DocEntry"=T1."DocEntry" join inv1 T2 on T1."TrgetEntry"=T2."BaseEntry" join oinv T3 on T2."DocEntry"=T3."DocEntry" LEFT OUTER JOIN INV9 T4 ON T3."DocEntry" = T4."DocEntry" left join odpi T5 on T4."BaseAbs" = T5."DocEntry" where T0."DocNum" in (' + req.body.inv + ') and T0."CANCELED" = ' + "'" + 'N' + "'" + ' order by T0."DocNum"';
