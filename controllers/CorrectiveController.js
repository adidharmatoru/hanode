...
exports.corrective = function(req, res, next) {
  ...
  var sql = 'select   T0."DocNum" as "Nomor SC",   T0."callID",   T0."custmrName",   T0."subject" as "subject",   T1."Name",   T2."DocAbs",   T5."Name" as "contactperson",   T4."DocNum" as "NomorSO",   T0."itemName" as "ItemName",   T0."itemCode" as "ItemCode",   T0."Series" as "Series",   T0."internalSN" as "SerialNum",   T0."BPShipAddr" as "Address",   T0."BPPhone1" as "Phone",   T4."U_VIT_TOPY" as "TOP",   T4."DocTotal" as "Total",   SUBSTRING(T7."DocDate", 1, 10) as "DocDateDO",   T7."DocNum" as "DocNumDO",   SUBSTRING(T4."DocDate", 1, 10) as "DocDateSO",   T4."DocNum" as "DocNumSO",   SUBSTRING(T0."createDate", 1, 10) as "createDate",   SUBSTRING(T0."closeDate", 1, 10) as "closeDate",   AT."AbsEntry" as "AbsEntry",   AT."FileExt" as "FileExt",   AT."FileName" as "FileName",   AT."trgtPath" as "targetPath" from   oscl T0   join oscs T1 on T0."status" = T1."statusID"   left join scl4 T2 on T0."callID" = T2."SrcvCallID"   left join rdr1 T3 on T2."DocAbs" = T3."DocEntry"   left join ordr T4 on T3."DocEntry" = T4."DocEntry"   left join ocpr T5 on T0."contctCode" = T5."CntctCode"   left join dln1 T6 on T2."DocAbs" = T6."BaseEntry"   left join odln T7 on T6."DocEntry" = T7."DocEntry"   left join ATC1 AT on AT."AbsEntry" = T0."AtcEntry" where   T0."Series" = ' + "' " + req.query.series + " '" + ' order by   T0."DocNum" DESC limit   700 ';
