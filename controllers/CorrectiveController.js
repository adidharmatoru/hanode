...
exports.Inv = function(req, res, next) {
  ...
  var sql = 'select distinct T0."DocNum" as "SO_no", T0."NumAtCard" as "No_PO", T3."DocNum" as "no_inv", T0."U_VIT_TOPY" as "TOP", T0."DocTotal" as "Total_so", T3."CardName" as "CardNameInvoice", SUBSTRING(T3."DocDate", 1, 10) as "inv_date", T3."DocStatus" as "status", T3."DocTotal" as "Total",T5."DocNum" as "no_dp",T5."DocTotal" as "Total_dp",T3."DocType" as "Typeitem", T7."DocNum" as "DocNumInpay" from ordr T0 join rdr1 T1 on T0."DocEntry"=T1."DocEntry" left join inv1 T2 on T1."TrgetEntry"=T2."BaseEntry" left join oinv T3 on T2."DocEntry"=T3."DocEntry" LEFT OUTER JOIN INV9 T4 ON T3."DocEntry" = T4."DocEntry" left join odpi T5 on T4."BaseAbs" = T5."DocEntry" left join rct2 T6 on T5."DocEntry" = T6."DocEntry" left join orct T7 on T6."DocNum" = T7."DocEntry" where T0."DocNum" in (' + req.body.inv + ') and T0."CANCELED" = ' + "'" + 'N' + "'" + ' order by T0."DocNum"';
