...
exports.ecDetail = function(req,res,next) {
  ...
  var sql = 'select   oins."insID" as "idEC",   oins."customer" as "cardCust",   oins."custmrName" as "Customer Name",   "internalSN" as "Serial Number",   oins."wrrntyStrt" as "Start Warranty",   oins."wrrntyEnd" as "End Warranty",   oins."itemCode" as "Item Code",   oins."itemName" as "Item Name",   oins."instLction" as "Location",   oins."contactCod",   ocpr."Name",   ocpr."Tel1",   ocpr."Tel2",   ocpr."E_MailL",   SUBSTRING(oins."createDate", 1, 10) as "createDate",   SUBSTRING(oins."dlvryDate", 1, 10) as "delivery DateEq", SUBSTRING(odln."DocDueDate", 1, 10) as "delivery Date" ,oitm."ItmsGrpCod" as "itemgroup",   oins."deliveryNo" as "deliveryNo",   oins."street" as "street",   oins."status" as "Status",   oins."block" as "block",   oins."zip" as "zip",   oins."city" as "city",   oins."country" as "country",   oins."county" as "county",   oins."state" as "state",   odln."U_VIT_WGSA" as "WarrantySparePart",   odln."U_VIT_WGBA" as "WarrantyBattery",   odln."U_VIT_WGSE" as "WarrantyService", dln1."Project" as "Project" from   oins   left join odln on oins."delivery" = odln."DocEntry"   left join ocpr on oins."contactCod" = ocpr."CntctCode" left join oitm on oins."itemCode" = oitm."ItemCode" left join dln1 on odln."DocEntry" = dln1."DocEntry" and dln1."LineNum" = 1 where oins."internalSN" =' + "'" + req.query.code + "'" + '';
  connection.runQuery(res, sql);
}

exports.eqValidasi = function(req,res,next) {
  res.setHeader('Access-Control-Allow-Origin', '*');

  var sql = 'select oins."insID" as "idEC",oins."customer" as "cardCust", oins."custmrName" as "Customer Name","internalSN" as "Serial Number",oins."wrrntyStrt" as "Start Warranty",oins."wrrntyEnd" as "End Warranty",oins."itemCode" as "Item Code",oins."itemName" as "Item Name",oins."instLction" as "Location",SUBSTRING(oins."createDate",1,10) as "createDate",SUBSTRING(oins."dlvryDate",1,10) as "delivery DateEq", SUBSTRING(odln."DocDueDate", 1, 10) as "delivery Date",oins."deliveryNo" as "deliveryNo",oins."street" as "street",oins."status" as "Status", oins."block" as "block",oins."zip" as "zip",oins."city" as "city",oins."country" as "country",oins."county" as "county",oins."state" as "state", odln."U_VIT_WGSA" as "WarrantySparePart",   odln."U_VIT_WGBA" as "WarrantyBattery", odln."U_VIT_WGSE" as "WarrantyService",dln1."Project" as "Project"  from oins left join odln on oins."delivery" = odln."DocEntry" left join dln1 on odln."DocEntry" = dln1."DocEntry" where oins."insID" =' + "'" + req.query.code + "'" + '';
  connection.runQuery(res, sql);
}
...
exports.ecScall = function(req,res,next) {
  ...
  var sql = 'select   C1."callID" AS "callID",   C1."DocNum" as "DocNum",   SUBSTRING(C1."createDate", 1, 10) as "createDate",   C1."subject" as "subject",   AT."AbsEntry" as "AbsEntry",   AT."FileExt" as "FileExt",   AT."FileName" as "FileName",   AT."trgtPath" as "targetPath",  C4."DocAbs",   D1."DocNum" as "DocNumDO",   AT2."AbsEntry" as "AbsEntryDO",   AT2."FileExt" as "FileExtDO",   AT2."FileName" as "FileNameDO",   AT2."trgtPath" as "targetPathDO", D2."ItemCode", D2."Dscription" as "ItemDelivery",D1."DocNum" as "DocNumDO" , D1."U_VIT_WGBA" as "WarrantyBat", SUBSTRING(D1."DocDate", 1, 10) AS "DocDateDO" from   oscl C1   left join ATC1 AT on AT."AbsEntry" = C1."AtcEntry"   left join scl4 C4 on C1."callID" = C4."SrcvCallID"  left join odln D1 on C4."DocAbs" = D1."DocEntry" and C1."customer" = D1."CardCode" left join dln1 D2 on D1."DocEntry" = D2."DocEntry" left join ATC1 AT2 on AT2."AbsEntry" = D1."AtcEntry" where   C1."internalSN" =' + "'" + req.query.code + "'" + ' AND C4."Object" = ' + "'" + '15' + "'" + ' and D1."CANCELED" = ' + "'" + 'N' + "'" + '';
  connection.runQuery(res, sql);
}
...
exports.ecScon = function(req,res,next) {
  ...
  var sql = 'select octr."ContractID",SUBSTRING(octr."StartDate", 1,10) AS "StartDate",SUBSTRING(octr."EndDate", 1, 10) as "EndDate",SUBSTRING(octr."TermDate", 1,10) as "TermDate",octr."SrvcType", octr."CntrcTmplt" as "TypeContract",ctr1."ItemCode" as "ItemCode",ctr1."ItemName" as "ItemName" from octr left join ctr1 on octr."ContractID"=ctr1."ContractID" where ctr1."InternalSN"=' + "'" + req.query.code + "'" + '';
  connection.runQuery(res, sql);
}
...
exports.ecTransaction = function(req,res,next) {
  ...
  var sql = 'select   T0."ItemCode",   T2."LogEntry",   SUBSTRING(T2."CreateDate", 1, 10) as "CreateDate",   T2."DocNum",   SUBSTRING(T2."DocDate", 1, 10) as "DocDate",   T2."LocCode",   T2."CardCode",   T2."CardName",   T2."DocType" ,  AT2."AbsEntry" as "AbsEntryDO",   AT2."FileExt" as "FileExtDO",   AT2."FileName" as "FileNameDO",   AT2."trgtPath" as "targetPathDO"from   osrn T0   left join itl1 T1 on T0."AbsEntry" = T1."MdAbsEntry"   left join oitl T2 on T1."LogEntry" = T2."LogEntry"   left join odln D1 on T2."DocNum" = D1."DocNum" and T2."DocType" = 15  left join ATC1 AT2 on AT2."AbsEntry" = D1."AtcEntry"where T0."DistNumber"=' + "'" + req.query.code + "'" + ' order by T2."CreateDate" ASC';
  connection.runQuery(res, sql);
}
...
exports.itemName = function(req,res,next) {
  ...
  var sql = 'select "ItemCode","ItemName","OnHand","DfltWH" as "Warehouse" from oitm where "ItemCode"  LIKE ' + "'%" + req.query.code + "%'" + ' order by "ItemName" DESC';
  connection.runQuery(res, sql);
}
...
exports.itemroxy = function(req, res, next){
  ...
  var sql = 'select   	oitw."ItemCode",  	oitm."ItemName",  	oitw."WhsCode",   	oitw."OnHand",  	oitw."IsCommited",   	oitw."OnOrder"   	from oitw   	left join oitm on oitm."ItemCode" = oitw."ItemCode" where "WhsCode" LIKE ' + "'" + '%RXY%' + "'" + 'and oitm."ItmsGrpCod" != 233';
...
exports.itemktb = function(req, res, next){
  ...
  var sql = 'select   	oitw."ItemCode",  	oitm."ItemName",  	oitw."WhsCode",   	oitw."OnHand",  	oitw."IsCommited",   	oitw."OnOrder"   	from oitw   	left join oitm on oitm."ItemCode" = oitw."ItemCode" where "WhsCode" LIKE  ' + "'" + '%KTB%' + "'" + 'and oitm."ItmsGrpCod" != 233';
