...
exports.ecDetail = function(req,res,next) {
  ...
  var sql = 'select oins."insID" as "idEC",oins."customer" as "cardCust", oins."custmrName" as "Customer Name","internalSN" as "Serial Number",oins."wrrntyStrt" as "Start Warranty",oins."wrrntyEnd" as "End Warranty",oins."itemCode" as "Item Code",oins."itemName" as "Item Name",oins."instLction" as "Location",SUBSTRING(oins."createDate",1,10) as "createDate",SUBSTRING(oins."dlvryDate",1,10) as "delivery Date",oins."deliveryNo" as "deliveryNo",oins."street" as "street",oins."block" as "block",oins."zip" as "zip",oins."city" as "city",oins."country" as "country",oins."county" as "county",oins."state" as "state", odln."U_VIT_WGSA" as "WarrantySparePart",   odln."U_VIT_WGBA" as "WarrantyBattery", odln."U_VIT_WGSE" as "WarrantyService"  from oins left join odln on oins."delivery" = odln."DocEntry"  where "internalSN" =' + "'" + req.query.code + "'" + ' and "status" = ' + "'" + 'A' + "'" + '';
  connection.runQuery(res, sql);
}
