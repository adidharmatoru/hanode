...
res.setHeader('Access-Control-Allow-Origin', '*');
  ...
  var sql = 'SELECT   distinct T4."PrjCode" as "PrjCode",   T0."ItemCode" as "ItemCode",   T0."Dscription" as "ItemName",   T0."Quantity",   T0."UomCode",  T1."NumAtCard" as "Nomor PO",  T1."CardName",  T1."DocNum" ,  SUBSTRING(T1."CreateDate",1,10) as "CreateDate", SUBSTRING(T1."DocDueDate",1,10) as "DocDueDate",  T1."U_VIT_TOPY" as "TOP",  T0."WhsCode",  T3."ItmsGrpNam", T5."lastName" as "lastname",T5."firstName" as "firstName"from RDR1 T0   JOIN ORDR T1 ON T1."DocEntry" = T0."DocEntry"  left join oitm T2 on T0."ItemCode" = T2."ItemCode"  left join oitb T3 on T2."ItmsGrpCod" = T3."ItmsGrpCod"  left join oprj T4 on T0."Project" = T4."PrjCode"  left join ohem T5 on T1."OwnerCode" = T5."empID" where   T4."PrjCode" is not null  AND NOT T0."ItemCode" =' + "'" + 'SC' + "'" + ' and not T0."ItemCode"=' + "'" + 'KG' + "'" + ' and not "U_Status" in ('+ "'D'" +','+ "'C'" +') and left(T4."PrjCode", 2) in (' + req.query.year + ') or SUBSTRING(T4."U_StartDate", 3, 2) in  (' + req.query.year + ') order by T4."PrjCode" desc  '
    connection.runQuery(res, sql);
}
